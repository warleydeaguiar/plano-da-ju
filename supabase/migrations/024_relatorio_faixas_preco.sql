...
as $$
  ...
  select coalesce(a.faixa, p.faixa) as faixa,
         case coalesce(a.faixa, p.faixa)
           when 'ate_100' then 3490 when 'ate_300' then 3990
           when 'ate_600' then 4490 when 'ate_1000' then 4990
   ...
   order by case coalesce(a.faixa, p.faixa)
              when 'ate_100' then 1 when 'ate_300' then 2 when 'ate_600' then 3
              when 'ate_1000' then 4 else 5 end;
$$;
