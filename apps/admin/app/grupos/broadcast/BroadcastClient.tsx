    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            {broadcast.title && (
              <div style={{ fontSize: 16, fontWeight: 700, color: '#2A1E2C', marginBottom: 4 }}>{broadcast.title}</div>
            )}
          ...
          <button
            onClick={onClose}
            style={{ background: '#FFFAF5', border: 'none', cursor: 'pointer', padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, color: '#2A1E2C', flexShrink: 0, marginLeft: 16 }}
          >
            Fechar
          </button>
        </div>
        ...
        <div style={{
          background: '#FFFAF5', borderRadius: 12, padding: '16px 18px',
          fontSize: 14, lineHeight: 1.7, color: '#2A1E2C', whiteSpace: 'pre-wrap', wordBreak: 'break-word',
        }}>
          {broadcast.message}
        </div>
  ...
  return (
    ...
    >
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, padding: '28px 32px', maxWidth: 480, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: '#2A1E2C', marginBottom: 16 }}>💾 Salvar mensagem</div>
        <div style={{ background: '#FFFAF5', borderRadius: 10, padding: '12px 14px', fontSize: 13, color: gray, marginBottom: 20, maxHeight: 100, overflow: 'hidden', lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ background: '#FFFAF5', border: 'none', cursor: 'pointer', padding: '10px 20px', borderRadius: 10, fontSize: 14, fontWeight: 600 }}>Cancelar</button>
          <button
    ...
    <div style={{ padding: '32px 40px', maxWidth: 980 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 330px', gap: 24, alignItems: 'start' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', padding: '24px' }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#2A1E2C', marginBottom: 20 }}>Nova mensagem</div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                {([['none', 'Sem mídia'], ['upload', '📎 Upload'], ['url', '🔗 URL']] as const).map(([k, l]) => (
                  <button key={k} onClick={() => { setMediaTab(k); if (k !== 'upload') removeMedia() }}
                    style={{ padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer', background: mediaTab === k ? accent : '#FFFAF5', color: mediaTab === k ? '#fff' : '#2A1E2C' }}>
                    {l}
                  </button>
                ))}
              </div>
                ...
                <div>
                  {!uploadedMedia ? (
                    <div onClick={() => fileInputRef.current?.click()}
                      ...
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#2A1E2C', marginBottom: 4 }}>Clique para selecionar imagem ou vídeo</div>
                    ...
                    <div style={{ border: `1px solid ${green}40`, borderRadius: 12, padding: '14px 16px', background: green + '06', display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#2A1E2C', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{uploadedMedia.filename}</div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {(['image', 'video'] as const).map(t => (
                      <button key={t} onClick={() => setMediaType(t)}
                        style={{ padding: '5px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', background: mediaType === t ? accent : '#FFFAF5', color: mediaType === t ? '#fff' : '#2A1E2C' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#2A1E2C' }}>💾 Mensagens salvas</div>
            ...
            {savedMessages.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {savedMessages.map(sm => (
                  <div key={sm.id} style={{ border: '1px solid #F0F0F5', borderRadius: 10, padding: '10px 12px', background: '#FFF7EE' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: '#2A1E2C', lineHeight: 1.3 }}>{sm.title}</div>
                      <button
          ...
          {/* Grupos alvo */}
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', padding: '20px' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#2A1E2C', marginBottom: 14 }}>Grupos que receberão</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {activeGroups.map(g => (
                  <div key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#2A1E2C' }}>{g.name}</div>
          ...
          {/* Números */}
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', padding: '20px' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#2A1E2C', marginBottom: 14 }}>Números</div>
            {instances.map(i => {
              ...
              return (
                <div key={i.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#2A1E2C' }}>{i.profileName ?? i.name}</div>
      ...
      {scheduled.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 14, border: `1px solid ${accent}20`, marginTop: 28 }}>
          <div style={{ padding: '18px 24px', borderBottom: '1px solid #F0F0F5', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#2A1E2C' }}>🕐 Mensagens agendadas</div>
              ...
              {scheduled.map(b => (
                <tr key={b.id} style={{ borderBottom: '1px solid #F9F9FC' }}>
                  <td style={{ padding: '12px 20px' }}>
                    {b.title && <div style={{ fontSize: 13, fontWeight: 700, color: '#2A1E2C', marginBottom: 2 }}>{b.title}</div>}
                    <div style={{ fontSize: 13, color: '#555', maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.message}</div>
                  </td>
                  <td style={{ padding: '12px 20px', fontSize: 12, color: '#2A1E2C', fontWeight: 600, whiteSpace: 'nowrap' }}>{b.scheduled_at ? formatDate(b.scheduled_at) : '—'}</td>
                  <td style={{ padding: '12px 20px', fontSize: 12, color: gray }}>{b.instance_name ?? '—'}</td>
                  <td style={{ padding: '12px 20px', fontSize: 12, color: '#2A1E2C' }}>{b.total_groups} grupos</td>
                  <td style={{ padding: '12px 20px' }}>
      ...
      {sent.length > 0 && (
        <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', marginTop: 28 }}>
          <div style={{ padding: '18px 24px', borderBottom: '1px solid #F0F0F5', fontSize: 15, fontWeight: 700, color: '#2A1E2C' }}>
            Histórico de envios
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: '#2A1E2C', background: '#FFF7EE',
        borderBottom: '1px solid #F0F0F5',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      }}>
        <span>👁 Preview da mensagem</span>
        <span style={{ fontSize: 9, color: '#7C6B7E', fontWeight: 500 }}>Como aparece no WhatsApp</span>
      </div>
      ...
      {/* Footer info */}
      <div style={{
        padding: '8px 12px', fontSize: 10, color: '#7C6B7E',
        background: '#FFF7EE', borderTop: '1px solid #F0F0F5',
        lineHeight: 1.4,
      }}>
        {hasMedia && hasText && (
        ...
        {mentionAll && (
          <div style={{ color: '#BE185D', fontWeight: 600 }}>🔔 Todos os membros serão mencionados</div>
        )}
      </div>
    </div>
  )
