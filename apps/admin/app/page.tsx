...
export default async function DashboardPage() {
  ...
  const [
    ...
    faixasPreco,
    // Alunas por dia (pagante × cortesia UGC)
    alunasDia,
  ] = await Promise.all([
    getDashboardStats(),
    getPendingPlans(8),
    ...
    // Alunas que entraram por dia, pagante × cortesia UGC (migração 030).
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (sb.rpc as any)('alunas_por_dia', { p_dias: 30 }),
  ]);
  ...
  const diasAlunas = (((alunasDia as { data?: DiaAlunas[] } | null)?.data) ?? []) as DiaAlunas[];
